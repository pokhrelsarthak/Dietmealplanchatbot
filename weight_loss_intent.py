...
import loss_utils as gu
from food import get_food

def compose_weight_loss(height, weight, gender, activity_level, age,intent_request, session_attributes):
    ...
    weight = float(weight)
    protein = gu.calculate_protein_loss(weight*2.2)
    fat = gu.calculate_fat_loss(calorie_subtracted)
    carbs = gu.caculate_carb_loss(calorie_subtracted, protein, fat)

    response1 = f'You are {status} and you need to consume {round(calorie_per_day,2)} calore per day to maintain this weight.'

    response2 = f'According to your goal, to loss weight you need to consume {round(calorie_subtracted,)} calorie per day, {round(protein,2)} gm protein per day, {round(fat, 2)} gm fat per day and {round(carbs,2)} carb per day'
...
def compose_food(food,intent_request):
    ...
    response_text = f'To loss weight you should adjust '
...
def handler(intent_request):
    ...
    if height and weight and gender and activity_level and age:
        if food is None:
            response, intent_request = compose_weight_loss(
